...
from libc.stdlib cimport malloc, free  # Example of importing memory functions

cpdef double my_sqrt(double x):
    return libc.math.sqrt(x)  # Call the C sqrt function


#ctypedef np.int_t DTYPE_t 
#ctypedef int DTYPE_t  # or ctypedef np.int32_t DTYPE_t, depending on your needs
ctypedef np.int32_t DTYPE_t  # Use np.int32_t to match the int32 data type


#helper function to calculate the distance between two points on a plane
cdef distance(Cpoint p1,Cpoint p2):
    cdef double dist = my_sqrt((p1.x-p2.x)*(p1.x-p2.x) + (p1.y-p2.y)*(p1.y-p2.y))
...
cdef class CshapeTriangle:
    ...
    cdef find_local_maximum(self,np.ndarray[DTYPE_t,ndim=2] distance_map):
        ...
        direction_x = x2-x1
        direction_y = y2-y1
        length = my_sqrt((y2-y1)**2 + (x2-x1)**2)
        
        #search along a line for a local maximum in the distance map
        while(my_sqrt((curr_y - y1)**2 + (curr_x - x1)**2) < length):
